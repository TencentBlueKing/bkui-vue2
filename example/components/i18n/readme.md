...
`bk-magic-vue` 组件库已支持国际化，默认的语言是中文，可以设置成其他语言。目前组件库支持中文 `zh-CN` 和英文 `en-US` 语言包。
...
<script>
    ...
    // 设置 bk-magic-vue 的语言包到 VueI18n 中
    const messages = {
...
### 使用 bk-magic-vue 以及 vue-i18n@6+ 来支持业务项目的国际化 {page=#/i18n}
...
使用 CDN 直接在页面上引入 `bk-magic-vue` 时，国际化本质上是一致的，只是获取 `lang`, `locale` 的方式有一些不一样，其他与上面一致。

```js
// 引入 bk-magic-vue 组件库 bk-magic-vue.min.js 后，页面上会存在 bkMagicVue 全局属性
